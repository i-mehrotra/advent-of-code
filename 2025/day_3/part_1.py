from typing import List

def parse_input(path: str) -> List[str]:
    banks: List[str] = []
    with open(path, "r") as file:
        for line in file:
            banks.append(line.strip())
    return banks

def get_highest_joltage(batteries: List[str]):
    highest = int(batteries[0])
    index = 0
    for i in range(1, len(batteries)):
        if int(batteries[i]) > highest:
            highest = int(batteries[i])
            index = i
    return highest, index

banks = parse_input("/Users/isabellamehrotra/Documents/advent-of-code/2025/day_3/input.txt")
highest_jolts: List[int] = []
for batteries in banks:
    highest, i = get_highest_joltage(batteries[:-1])
    second_highest, _ = get_highest_joltage(batteries[i+1:])
    highest_jolts.append(int(str(highest) + str(second_highest)))

sum = 0
for jolts in highest_jolts:
    sum += jolts
print(sum)